...
**Pont pas encore construit** : le scraper envoie pour l'instant ses alertes uniquement sur le Telegram personnel de Justok. Il faudra ajouter côté scraper un module équivalent à `connecteur_supabase.py` (pokedeals) qui écrit dans la table `precommande_alerts` de CE projet Supabase (et notifie push/email les abonnés, équivalent à `notifications_saas.py`) — nécessite les secrets `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` de CE projet configurés côté scraper (distincts de ceux de pokedeals-saas).
